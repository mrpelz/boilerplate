 ...
 */
export const getProcessedBody = (input) => {
  const lines = input
    .trim()
    .split('\n')
    .map((line) => line.trim());
...
export const getEnvironment = () => {
  ...
  console.info({
    CI_API_V4_URL: environment.apiUrl,
    CI_BOT_TOKEN: environment.token,
    CI_PIPELINE_ID: environment.pipelineId,
    CI_PROJECT_ID: environment.projectId,
  });
...
export const replaceComments = async (
  ...
  console.info(`Found ${matchingComments.length} matching comment(s).`);
