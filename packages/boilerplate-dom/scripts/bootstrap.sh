...
if [[ $SKIP_FILES -ne 1 ]]; then
	if check_response "🖇 install symbolic links referencing files in \"$BOILERPLATE_MODULE_NAME\" and \"$BOILERPLATE_DOM_MODULE_NAME\"?" y; then
		make_ln "${BOILERPLATE_DOM_MODULE_PATH}/.editorconfig" .editorconfig

		make_ln "${BOILERPLATE_DOM_MODULE_PATH}/.shellcheckrc" .shellcheckrc

		make_ln "${BOILERPLATE_DOM_MODULE_PATH}/.vscode/extensions.json" .vscode/extensions.json
		make_ln "${BOILERPLATE_DOM_MODULE_PATH}/.vscode/settings.json" .vscode/settings.json
...
export default {
  extends: [
    resolve(
      dirname(fileURLToPath(import.meta.resolve('$BOILERPLATE_DOM_MODULE_NAME'))),
      'stylelint.config.js',
    ),
  ],
};
